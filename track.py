...
def separate_connected_components(img):
    """
    Separate disconnected regions that have the same label value.
    Returns a new image where each connected component gets a unique label.
    """
    unique_labels = np.unique(img)[1:]  # Exclude background (0)
    result = np.zeros_like(img, dtype=np.uint32)
    next_label = 1
    
    for label in unique_labels:
        # Create binary mask for current label
        binary_mask = (img == label).astype(np.uint8)
        
        # Find connected components within this label
        labeled_components = connected_components(binary_mask, connectivity=2)
        
        # Assign unique labels to each component
        component_labels = np.unique(labeled_components)[1:]  # Exclude background
        
        for comp_label in component_labels:
            component_mask = (labeled_components == comp_label)
            result[component_mask] = next_label
            next_label += 1
...
def predict_dataset_2(path, output_path, threshold=0.1):
    ...
    for i, name in enumerate(names):
        ...
        # 🔧 FIX: Separate disconnected components before tracking
        img_separated = separate_connected_components(img)

        labels = np.unique(img_separated)[1:]
        ...
        # store result
        cv2.imwrite(os.path.join(output_path, "predict"+name), result.astype(np.uint16))
        old = result
